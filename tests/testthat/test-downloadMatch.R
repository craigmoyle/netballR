test_that("build_match_url validates and formats request identifiers", {
  expect_equal(
    netballR:::build_match_url("10083", 5, 3),
  ...
  expect_error(
    netballR:::build_match_url("season-2025", 5, 3),
    "comp_id must contain digits only"
  )
  expect_error(
    netballR:::build_match_url("10083", 0, 3),
    "round_id must be greater than or equal to 1"
  )
  expect_error(
    netballR:::build_match_url("10083", 5, 1.5),
...
test_that("extract_match_stats fails loudly when matchStats is absent", {
  ...
  expect_equal(netballR:::extract_match_stats(payload), payload$matchStats)
  expect_error(
    netballR:::extract_match_stats(list()),
